...
<style>${ARIADNE_CSS_VARIABLES}body{margin:0;background:var(--ariadne-canvas);color:var(--ariadne-report-foreground);font:15px/1.55 system-ui,sans-serif}main{max-width:1050px;margin:auto;padding:36px 20px}h1{font-size:2rem}h1,h2,h3,h4,a{color:var(--ariadne-accent)}h2{margin-top:2.5rem}.summary,.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px}.card,section{background:var(--ariadne-surface);border:1px solid var(--ariadne-border);border-radius:8px;padding:16px;margin-top:12px}.card strong,dt{display:block;color:var(--ariadne-accent);font-size:.78rem;text-transform:uppercase}dd{margin:0 0 8px}pre{overflow:auto;white-space:pre-wrap;background:var(--ariadne-code-background);color:var(--ariadne-code-foreground);padding:12px;border-radius:6px}.fail{border-left:5px solid var(--ariadne-error)}.warning,.warning strong{color:var(--ariadne-warning)}.warning{border-left:5px solid var(--ariadne-warning)}.pass{border-left:5px solid var(--ariadne-pale-slate)}table{width:100%;border-collapse:collapse}th,td{text-align:left;border-bottom:1px solid var(--ariadne-border);padding:8px}code{overflow-wrap:anywhere}@media print{body{background:var(--ariadne-snow)}details{display:block}section,.card{break-inside:avoid}}</style></head><body><main>
...
${model.warnings.map((warning) => `<p class="card warning"><strong>Warning</strong>${escapeHtml(warning)}</p>`).join("")}
