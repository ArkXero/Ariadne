import { resumeWorkflow } from "../core/workflow-control.js";
...
export async function resumeCommand(options: { cwd: string; batchId: string; configPath?: string; concurrency?: number; allowDirtyBase?: boolean; json?: boolean; quiet?: boolean }): Promise<{ batch: BatchRecord; signal?: NodeJS.Signals }> {
  const execution = await withWorkflowSignals((signal) => resumeWorkflow({
    cwd: options.cwd, batchId: options.batchId, configPath: options.configPath, concurrency: options.concurrency, allowDirtyBase: options.allowDirtyBase, signal,
