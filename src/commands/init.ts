...
const CONFIG_TEMPLATE = `version: 2
...
export function formatInitResult(result: InitResult): string {
  return [`ariadne.yml ${result.config}`, `.ariadne/tasks/example.yml ${result.task}`, `.ariadne/runs ${result.runsDirectory}`, `.gitignore ${result.gitignore}`].join("\n");
}
